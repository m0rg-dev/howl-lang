    ...
    apply() {
        this.ApplySingleProductionRule({
            ...
            replace: (ast_stream: [ClassHeaderElement, ...ASTElement[]]) => {
                ...
                const cl = new ClassElement(ast_stream[0].source_location, ast_stream[0].name, fields, methods, ast_stream[0].generics, ast_stream[0].parent, ast_stream[0].interfaces, ast_stream[0].is_interface);
                return [cl];
            }
        });
    }
