use crate::ast::{
    ...
    ELSE_IF_STATEMENT_BODY, ELSE_IF_STATEMENT_CONDITION, ELSE_STATEMENT_BODY,
    FIELD_REFERENCE_EXPRESSION_SOURCE, FUNCTION_BODY, FUNCTION_CALL_EXPRESSION_SOURCE,
...
pub fn pretty_print(source: ASTElement) -> String {
    match &source.element() {
        ...
        ASTElementKind::FieldReferenceExpression { span: _, name } => {
            format!(
                "({}).{}",
                pretty_print(source.slot(FIELD_REFERENCE_EXPRESSION_SOURCE).unwrap()),
                name
            )
        }
