        ...
        if (e instanceof FunctionCallExpression) {
            ...
            if (source instanceof FieldReferenceExpression) {
                ...
                if (source_type instanceof ClassStaticType) {
                    ...
                    List<Function> candidates = ((ClassStaticType) source_type).getSource()
                            .getOverloadCandidates(frex.getName());
                    List<Function> matches = new ArrayList<>();
                    for (Function candidate : candidates) {
                        List<String> candidate_types = new ArrayList<>();
                        boolean all_match = true;
                        for (int i = 0; i < (((FunctionType) candidate.getOwnType()).getArgumentTypes()).size(); i++) {
                            TypeElement argtype = (((FunctionType) candidate.getOwnType()).getArgumentTypes()).get(i)
                                    .resolve();
