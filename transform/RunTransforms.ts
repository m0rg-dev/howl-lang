...
export function RunFunctionTransforms(f: FunctionElement) {
    log(LogLevel.TRACE, `FunctionTransforms ${f}`, `~~~ RunFunctionTransforms ${f} ~~~`);

    WalkAST(f, (x) => {
        if (x instanceof StringConstantExpression) {
            log(LogLevel.TRACE, `FunctionTransforms ${f}`, `{ConvertStringConstant} ${x}`);
    ...
    WalkAST(f, (x) => {
        if (x instanceof AssignmentStatement
            ...
            log(LogLevel.TRACE, `FunctionTransforms ${f}`, `{Replace__L_Index__} ${x}`);
    ...
    WalkAST(f, (x) => {
        if (x instanceof IndexExpression
            ...
            log(LogLevel.TRACE, `FunctionTransforms ${f}`, `{Replace__Index__} ${x}`);
    ...
    WalkAST(f, (x) => {
        if (x instanceof FunctionCallExpression
            ...
            log(LogLevel.TRACE, `FunctionTransforms ${f}`, `{AddSelfToMethodCall} ${x}`);
    ...
    WalkAST(f, (x) => {
        if (x instanceof FieldReferenceExpression
            ...
            log(LogLevel.TRACE, `FunctionTransforms ${f}`, `{IndirectMethodReference} ${x}`);
