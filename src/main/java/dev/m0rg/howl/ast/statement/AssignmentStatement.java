    ...
    public void generate(LLVMFunction f) {
        if (this.getLHS() instanceof Lvalue) {
            ...
            try (LLVMBuilder builder = new LLVMBuilder(f.getModule())) {
                ...
                if (this.getRHS() instanceof MacroCallExpression
                        && ((MacroCallExpression) this.getRHS()).getName().equals("typednull")) {
                    ALambdaTerm target_type = ALambdaTerm.evaluateFrom(this.getLHS());

                    builder.buildStore(target_type.toLLVM(f.getModule()).getNull(f.getModule()), l.getPointer(builder));
                } else {
