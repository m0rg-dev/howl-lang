...
export class Terminal {
    ...
    protected match_keyword(): Token | undefined {
        const m = this.source.substr(this.start).match(/^(class|fn|i32|return)\s+/s);
    ...
    protected match_punctuation(): Token | undefined {
        const m = this.source.substr(this.start).match(/^([{};()])\s*/s);
        if (!m) return undefined;
    ...
    protected match_name(): NameToken | undefined {
        const m = this.source.substr(this.start).match(/^([_a-zA-Z][_a-zA-Z0-9-]*)\s*(?:\s|(?=[^_a-zA-Z0-9-]))/s);
        if (!m) return undefined;
    ...
    protected match_numeric(): NumericLiteralToken | undefined {
        const m = this.source.substr(this.start).match(/^(\d+)\s*(?:\s|(?=[^_a-zA-Z0-9-]))/s);
        if (!m) return undefined;

        return { type: TokenType.NumericLiteral, value: Number.parseInt(m[1]), length: m[0].length };
    }

    protected next_token(): Token | undefined {
